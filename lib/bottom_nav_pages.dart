...
class Home_State extends State<Home> {
  ...
  final subcatref = FirebaseFirestore.instance.collection('subcat');
  @override
  void initState() {
    getcategory();
    super.initState();
  ...
  void getcategory() async {
    ...
    await for (var snapshot in catref) {
      for (var message in snapshot.docs) {
        //print(message.data());

        category.add(message.data());
      }
    }
  }

  void getsubcat(val, cat) async {
    if (val == true) {
      Stream catref = FirebaseFirestore.instance
          .collection('subcat')
          .where('catid', isEqualTo: cat)
          .snapshots();
      await for (var snapshot in catref) {
        for (var message in snapshot.docs) {
          // print(message.data());
          setState(() {
            subcategory.add(message.data());
          });
        }
      }
    } else {
                    ...
                    Container(
                      ...
                      child: ExpansionTile(
                        onExpansionChanged: (value) {
                          ...
                          getsubcat(value, category[i]['catid']);
                        },
                        backgroundColor: Colors.blue[100],

                        textColor: Colors.black,
                        leading: Icon(Icons.admin_panel_settings_outlined),
                        //onExpansionChanged:(bool expanding)=> _onExpansion(category[i]['catname']),
